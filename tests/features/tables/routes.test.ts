 ...
 * service (pipeline real de tabelas, que abriria conexão MySQL) e sessionStore.
 */
...
const previewTables = vi.fn(async () => [{ name: "t1", issues: [] }]);
...
vi.mock("../../../src/features/tables/service.js", () => ({
  previewTables: (...args: unknown[]) => previewTables(...(args as [])),
  runTablesJob: vi.fn(),
}));
...
describe("POST /tables/jobs", () => {
  ...
  it("mesmo par de perfis com bancos diferentes gera dois jobs, cada um com o seu banco", async () => {
    const app = buildApp();
    await app.inject({ method: "POST", url: "/tables/jobs", cookies: ANA, payload: jobBody });
    await app.inject({
      method: "POST",
      url: "/tables/jobs",
      cookies: ANA,
  ...
  it("sem sessão: 401", async () => {
    const app = buildApp();
    const res = await app.inject({ method: "POST", url: "/tables/jobs", payload: jobBody });
